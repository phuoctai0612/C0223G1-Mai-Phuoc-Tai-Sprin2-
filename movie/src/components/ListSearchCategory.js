...
import {
    getCategorys,
    getListSearchAuthor,
    getListSearchCategory, getListSearchMovieName,
    getListSearchPlus,
    getNations
...
export default function ListSearchCategory() {
    const param = useParams();
    const [nations, setNations] = useState([]);
    const [categorys, setCategorys] = useState([]);
    const [listSearchPlus, setListSearchPlus] = useState([]);
    const [categoryDisplay, setCategoryDisplay] = useState(0);
    const [categorySearch, setCategorySearch] = useState(0);
    const [categorySearch1, setCategorySearch1] = useState(param.id);
...
const getCategoryDisplay=async ()=>{
        setCategoryDisplay(await getCategorysById(param.id))
}
    const nextPage = async () => {
        const newPage = pageSearch + 1;
        const result = await getListSearchAuthor(newPage, typeMovieSearch, categorySearch,categorySearch1, nationSearch, yearStartSearch)
        if (result.length != 0) {
            window.scrollTo(0, 0)

            await getPageSearch(newPage).then(setListSearchPlus(await getListSearchCategory(newPage, typeMovieSearch, categorySearch,categorySearch1, nationSearch, yearStartSearch)))
        }
    }
    ...
    const handlePageChange = async (data) => {
        window.scrollTo(0,0)
        await getPageSearch(data.selected).then(setListSearchPlus(await getListSearchCategory(data.selected, typeMovieSearch, categorySearch,categorySearch1, nationSearch, yearStartSearch)))
    ...
    useEffect(() => {
        setCategorySearch1(param.id)
        getListSearch(pageSearch, typeMovieSearch, categorySearch,categorySearch1, nationSearch, yearStartSearch)
    }, [categorySearch1])
    useEffect(() => {
     setTimeout(()=>{
         setCategorySearch1(param.id)
    ...
    const getListSearch = async (pageSearchNew, typeMovieSearchNew, categorySearchNew,categorySearchNew1, nationSearchNew, yearStartSearchNew) => {
        setListSearchPlus(await getListSearchCategory(pageSearchNew, typeMovieSearchNew, categorySearchNew,categorySearchNew1, nationSearchNew, yearStartSearchNew));
    }
    ...
    const handleSearch = async () => {
        ...
        await getTypeMovieSearch(typeMovieSortInput)
            .then(await getCategorySearch(categoryInput))
            .then(await getNationSearch(nationInput))
            .then(await getYearStartSearch(yearStartInput))
            .then(await getPageSearch(pageInput))
            .then(await getListSearch(pageInput, typeMovieSortInput, categoryInput,categorySearch1, nationInput, yearStartInput))
    }
                            ...
                            <div className="product__page__filter" style={{paddingBottom: "40px"}}>
                                <select className="styleSearch" id={"category"} onChange={() => {
                                    ...
                                    categorys.map((item, index) =>
                                        <option key={`categorys_${index}`} value={item.id}>{item.name}</option>
                                    )
                                    }
                                </select>
                            </div>
